...
from .cbam_resnet import cba_resnet50
from .circle_layer import Circle

__all__ = ['CBAMGResNet50V3a','MGResNet50V3', 'MGResNet50V3a', 'MGResNet50V3a_ibn_a', 'MGResNet50V3a_ibn_b', 'MGSEResNet50V3', 'MGSEResNet50V3a', 'MGSEResNet50V3b', 'MGSEResNet50V3c', 'MGSEResNet50V3d','MGResNet50V3x', 'MGResNet50V3x_1']
...
class MGBranchV3a(nn.Module):
    def __init__(self, num_classes, backbone='resnet50', parts_num=0, pool_channels=2048, embedding_dim=256, onnx_en=False):
        ...
        #    res50 = resnet50_ibn_b(pretrained=True)
            ...
            private_layer4 = res50.layer4
            private_conv = nn.Sequential(private_layer3, private_layer4)
        elif backbone == 'resnet50_ibn_b':
            res50 = resnet50_ibn_b(pretrained=True)
            private_layer3 = res50.layer3
            private_layer4 = res50.layer4
            private_conv = nn.Sequential(private_layer3, private_layer4)
        elif backbone == 'cba_resnet50':
            res50 = cba_resnet50(pretrained=True)
            private_layer3 = res50.layer3
            private_layer4 = res50.layer4
            private_conv = nn.Sequential(private_layer3, private_layer4)
        else:
        ...
        self.private_conv = private_conv
        if self.parts_num > 0:
            if backbone=='resnet50' or backbone == 'resnet50_ibn_a' or backbone == 'resnet50_ibn_b' or backbone== 'cba_resnet50':
                self.private_conv[1][0].conv2.stride = (1, 1)
            elif backbone=='seresnet50' :
                self.private_conv[1][0].conv1.stride = (1, 1)
            self.private_conv[1][0].downsample[0].stride = (1, 1)
        ...
        if onnx_en:
            ...
            global_pool = GeneralizedMeanPoolingP()
        else:
...
##
        ...
        if self.parts_num > 0:
            if onnx_en:
                ...
                self.adaptive_pool = GeneralizedMeanPoolingP(output_size = (self.parts_num, 1))
            else:
                self.adaptive_pool = nn.AdaptiveMaxPool2d((self.parts_num, 1))
            self.local_emb_name = {}
            self.local_bottleneck_name = {}
            self.local_cls_name = {}
            for i in range(self.parts_num):
                if onnx_en:
                ...
                setattr(self, self.local_emb_name[i], local_emb)
## BNNneck      
#                bottleneck = nn.BatchNorm1d(embedding_dim)
#                bottleneck.bias.requires_grad_(False)
#                bottleneck.apply(weights_init_kaiming)
#                self.local_bottleneck_name[i] = 'local_bottleneck%d'%(i+1)
#                setattr(self, self.local_bottleneck_name[i], bottleneck)
##
                if not onnx_en:
                    classifier = nn.Linear(embedding_dim, num_classes)
                    classifier.apply(weights_init_classifier)
    ...
    def forward(self, x):
        ...
        if self.training:
            global_feat = global_feat.view(global_feat.size(0), -1)
 #           temp_feat = self.bottleneck(global_feat)   # BNNeck
            global_score = self.classifier(global_feat)
        ...
        if self.parts_num > 0:
            ...
            for i in range(self.parts_num):
                ...
                if self.training:
                    feat = feat.view(feat.size(0), -1)
     #               extr_temp = getattr(self, self.local_bottleneck_name[i]) #BNNeck
     #               temp_local_feat = extr_temp(feat)                        #
                    cls = getattr(self, self.local_cls_name[i])                                               
                    score = cls(feat)
                    local_score_group.append(score)                                                 
                local_feat_group.append(feat)
        ...
        if self.training:
            return tuple([global_score] + local_score_group), tuple([global_feat] + local_feat_group)
        else:
            return tuple([global_feat] + local_feat_group)

        
        
class MGResNet50V3a_ibn_a(nn.Module):
    def __init__(self, num_classes, loss={'xent'}, branch_stripes=[0,2,3], **kwargs):
        super(MGResNet50V3a_ibn_a, self).__init__()
        self.loss = loss
        self.onnx_en = False
      ...
      #  res50 = resnet50_ibn_b(pretrained = True)
        ...
        for i, num in enumerate(branch_stripes):
            self.branch_name[i] = 'branch%d'%(i+1)
            setattr(self, self.branch_name[i], MGBranchV3a(num_classes, backbone='resnet50_ibn_a', parts_num=num, onnx_en=self.onnx_en))
...
class MGResNet50V3a_ibn_b(nn.Module):
    def __init__(self, num_classes, loss={'xent'}, branch_stripes=[0,2,3], **kwargs):
        super(MGResNet50V3a_ibn_b, self).__init__()
        self.loss = loss
        self.onnx_en = False 
      ...
      #  res50 = resnet50_ibn_a(1, pretrained = True)
        ...
        for i, num in enumerate(branch_stripes):
            self.branch_name[i] = 'branch%d'%(i+1)
            setattr(self, self.branch_name[i], MGBranchV3a(num_classes, backbone='resnet50_ibn_b', parts_num=num, onnx_en=self.onnx_en))
...
class CBAMGResNet50V3a(nn.Module):
    def __init__(self, num_classes, loss={'xent'}, branch_stripes=[0,2,3], **kwargs):
        super(CBAMGResNet50V3a, self).__init__()
        self.loss = loss

        res50 = cba_resnet50(pretrained = True)
#        self.common = nn.Sequential(seres50.base[0], seres50.base[1], seres50.base[2])

        self.common = nn.Sequential(res50.conv1, res50.bn1,res50.relu,res50.maxpool,res50.layer1, res50.layer2
        )
        self.branch_stripes = branch_stripes
        self.branch_name = {}
        for i, num in enumerate(branch_stripes):
            self.branch_name[i] = 'branch%d'%(i+1)
            setattr(self, self.branch_name[i], MGBranchV3a(num_classes, backbone='cba_resnet50', parts_num=num))
...
class MGBranchV3x(nn.Module):
    def __init__(self, num_classes, backbone='resnet50', parts_num=0, pool_channels=2048, embedding_dim=480, onnx_en=False):
        ...
        if self.parts_num > 0:
            ...
            if self.parts_num == 2:
                embedding_dim = 240
            elif self.parts_num == 3:
                embedding_dim = 160
            elif self.parts_num == 4:
                embedding_dim = 120
            self.local_emb_name = {}
            self.local_cls_name = {}
            for i in range(self.parts_num):
...
class MGResNet50V3x(nn.Module):
    def __init__(self, num_classes, loss={'xent'}, branch_stripes=[2,3,4], **kwargs):
...
class MGBranchV3x_1(nn.Module):
    def __init__(self, num_classes, backbone='resnet50', parts_num=0, pool_channels=2048, embedding_dim=480, onnx_en=False):
        super(MGBranchV3x_1, self).__init__()
        self.onnx_en = onnx_en
        self.parts_num = parts_num
        if self.parts_num <= 1:
            self.parts_num = 0
        ...
        if self.parts_num > 3:
            ...
            self.private_conv[0][0].downsample[0].stride = (1, 1)
        # global embedding
        self.private_layer3 = self.private_conv[0]
        if onnx_en:
            factor_dict = {0:1, 2:2, 3:4}
            factor = factor_dict[self.parts_num]
            pool_h = int(12 * factor)
        ...
        self.embedding_3 = nn.Sequential(
            global_pool,
            nn.Conv2d(1024, embedding_dim, kernel_size=1, bias=False),
            nn.BatchNorm2d(embedding_dim),
        )
        self.embedding_3.apply(weights_init_kaiming)
        self.embedding = nn.Sequential(
            global_pool,
            nn.Conv2d(pool_channels, embedding_dim, kernel_size=1, bias=False),
            nn.BatchNorm2d(embedding_dim),
    ...
    def forward(self, x):
        x_3 = self.private_layer3(x)
        x = self.private_conv(x)
        global_feat_3 = self.embedding_3(x_3)
        global_feat = self.embedding(x)
        if self.training:
            global_feat = global_feat.view(global_feat.size(0), -1)
            global_score = self.classifier(global_feat)
            global_feat_3 = global_feat_3.view(global_feat.size(0), -1)
            global_score_3 = self.classifier(global_feat_3)
        ...
        if self.training:
            return tuple([global_score_3, global_score] + local_score_group), tuple([global_feat_3, global_feat]), local_feat_group
        else:
            return tuple([global_feat_3, global_feat] + local_feat_group)

        
class MGResNet50V3x_1(nn.Module):
    def __init__(self, num_classes, loss={'xent'}, branch_stripes=[2,3,4], **kwargs):
        super(MGResNet50V3x_1, self).__init__()
        self.loss = loss
        self.onnx_en = False 
  ...
  #      res50 = resnet50_ibn_a(1, pretrained=True)
        ...
        for i, num in enumerate(branch_stripes):
            self.branch_name[i] = 'branch%d'%(i+1)
            setattr(self, self.branch_name[i], MGBranchV3x_1(num_classes, parts_num=num, onnx_en=self.onnx_en))
...
class MGSEResNet50V3x_1(nn.Module):
    def __init__(self, num_classes, loss={'xent'}, branch_stripes=[2,3,4], **kwargs):
        super(MGSEResNet50V3x_1, self).__init__()
        self.loss = loss
        ...
        for i, num in enumerate(branch_stripes):
            self.branch_name[i] = 'branch%d'%(i+1)
            setattr(self, self.branch_name[i], MGBranchV3x_1(num_classes, backbone='seresnet50', parts_num=num))
