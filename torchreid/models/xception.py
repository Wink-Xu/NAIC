...
class ExitFlow(nn.Module):
    def __init__(self, in_channels, nchannels, stride=2):
        super(ExitFlow, self).__init__()
        self.stride = stride
        self.conv1 = SepConv(in_channels, nchannels[0])
        self.conv2 = SepConv(nchannels[0], nchannels[1])
        self.conv2s = nn.Sequential(
            nn.Conv2d(in_channels, nchannels[1], 1, stride=self.stride, bias=False),
    ...
    def forward(self, x):
        ...
        x2 = F.max_pool2d(x2, 3, stride=self.stride, padding=1)
        x2s = self.conv2s(x)
        x2 = x2 + x2s
...
class XceptionNet(nn.Module):
    def __init__(self, num_classes=1000, loss={'xent'}, **kwargs):
        ...
        self.entryflow = EntryFLow(nchannels=[32, 64, 128, 256, 728])
        self.midflow = MidFlow(728, 728, 8)
        self.exitflow = ExitFlow(728, nchannels=[728, 1024, 1536, 2048], stride=1)
...
if __name__=='__main__':
    ...
    net = XceptionNet(1000, loss={'xent','htri'})
    ipdb.set_trace()
