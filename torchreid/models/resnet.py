...
import math
__all__ = ['ResNet50', 'ResNet101', 'ResNet50M', 'ResNet50V2', 'ResNet50V3', 'ResNet50V3MS', 'ResNet50V2a', 'ResNet50V3a', 'ResNet50V3aLg', 'ResNet101V3a']
...
def ResNet_more(pretrained=True, **kwargs):
    ...
    model = ResNet50_more(last_stride = 1, bn_norm="BN", num_splits = 1, with_ibn = ibn , with_se=0, with_nl=0, block= Bottleneck, layers = [3, 4, 6, 3], non_layers = [0, 2, 3, 0])	
    if pretrained:
        if not ibn:            
...
class ResNet50V2a(nn.Module):
    def __init__(self, num_classes, loss={'xent'}, **kwargs):
        super(ResNet50V2a, self).__init__()
        self.loss = loss
		
        self.base = ResNet_more(pretrained=True)
